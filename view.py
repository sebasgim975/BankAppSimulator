...
class View:
    def __init__(self, master):
        ...
        self.frase = tk.Label(self.frame, text="Sistema de Controlo de Finanças", font=(
        ...
        self.imagem_1_label = tk.Label(
            self.frame, image=self.imagem_1, bg="#92e3a9")
        ...
        self.registo_button = tk.Button(self.frame, text="Registo", bg="#2e5448", foreground="white", font=(
            "Elephant", 16), width=20, height=2, command=self.registar)
    ...
    def registar(self):
        ...
        tk.Label(self.nova_janela, text="Nome de utilizador", bg="#92e3a9", font=(
        ...
        self.registo_f_button = tk.Button(self.nova_janela, text="Registar", bg="#2e5448", foreground="white", font=(
            "Times New Roman", 12), width=10, command=self.confirmar_registar)
    ...
    def confirmar_registar(self):
        ...
        self.user_info = Cliente(self.nome_entry.get(
        ), self.password_entry.get(), self.nif_entry.get())
    ...
    def login(self):
        ...
        self.registo_f_button = tk.Button(self.nova_janela, text="Login", bg="#2e5448", foreground="white", font=(
            "Times New Roman", 12), width=10, command=self.confirmar_login)
    ...
    def confirmar_login(self):
        ...
        c.execute("SELECT * FROM user_data")
        self.saved_table = c.fetchall()
        self.user_info = Cliente(
            self.nome_entry.get(), self.password_entry.get(), None)
    ...
    def despesas(self):
        ...
        self.imagem_2_label = tk.Label(
            self.nova_janela, image=self.imagem_2, bg="#92e3a9")
        self.imagem_2_label.pack(pady=10)

        self.orcamento_button = tk.Button(self.nova_janela, text="Definir Orçamento", bg="#2e5448", foreground="white", font=(
            "Times New Roman", 14), width=30, command=self.orcamento_mensal)
        self.orcamento_button.pack(pady=5)

        self.adicionar_despesas_button = tk.Button(self.nova_janela, text="Adicionar despesas", bg="#2e5448", foreground="white", font=(
            "Times New Roman", 14), width=30, command=self.adicionar_despesas)
        self.adicionar_despesas_button.pack(pady=5)

        self.consultar_despesas_button = tk.Button(self.nova_janela, text="Consultar despesas", bg="#2e5448", foreground="white", font=(
            "Times New Roman", 14), width=30, command=self.consultar_despesas)
        self.consultar_despesas_button.pack(pady=5)

        self.gasto_mensal_button = tk.Button(self.nova_janela, text="Limite máximo de gastos", bg="#2e5448", foreground="white", font=(
            "Times New Roman", 14), width=30, command=self.limitar_gastos)
        self.gasto_mensal_button.pack(pady=5)

        self.analise_button = tk.Button(self.nova_janela, text="Analise das despesas", bg="#2e5448", foreground="white", font=(
            "Times New Roman", 14), width=30, command=self.analise)
    ...
    def adicionar_despesas(self):
        ...
        tk.Label(self.nova_janela, text="Categoria de despesa", bg="#92e3a9", font=(
            "Times New Roman", 15)).grid(row=0, column=0, sticky="w")
        self.categoria_de_despesa_entry = ttk.Combobox(self.nova_janela, values=[
                                                       "Casa", "Supermercado", "Carro", "Restaurante", "Lazer"])
        self.categoria_de_despesa_entry.grid(row=0, column=1, sticky="w")

        tk.Label(self.nova_janela, text="Descrição de despesa", bg="#92e3a9", font=(
            "Times New Roman", 15)).grid(row=1, column=0, sticky="w")
        self.descricao_de_despesa_entry = tk.Entry(self.nova_janela)
        self.descricao_de_despesa_entry.grid(row=1, column=1, sticky="w")

        tk.Label(self.nova_janela, text="Valor da despesa", bg="#92e3a9", font=(
            "Times New Roman", 15)).grid(row=2, column=0, sticky="w")
        self.valor_da_despesa_entry = tk.Entry(self.nova_janela)
        self.valor_da_despesa_entry.grid(row=2, column=1, sticky="w")

        tk.Label(self.nova_janela, text="Data da despesa", bg="#92e3a9", font=(
        ...
        self.adicionar_button = tk.Button(self.nova_janela, text="Adicionar", bg="gray", font=(
            "Arial", 12), width=10, command=self.error_checking_adicoes)
        ...
        if self.categoria_de_despesa_entry.get() == '' or self.descricao_de_despesa_entry.get() == '' or self.valor_da_despesa_entry.get() == '' or self.data_da_despesa_entry.get() == '':
            ...
            try:
                if c.execute("SELECT * FROM user_data WHERE Nome_do_utilizador = ?", (self.nome_actual,)).fetchone()[7] == '':
                    ...
                    c.execute(
                        "SELECT * FROM user_data WHERE Nome_do_utilizador = ?", (self.nome_actual,))
                    self.saved_table = c.fetchall()
                    ...
                    self.salario = Orcamento(
                        self.orcamento_inicial, self.gasto_maximo, self.valor_despesa_total, self.valor_despesa_total_gastos)
                ...
                self.orcamento_atual = self.salario.retirar(
                    self.salario.get_orcamento(), self.valor_da_despesa_atual)
                if self.salario.get_gasto_maximo() != 0:
                    self.salario.despesa_adicionar_gastos(
                        self.valor_da_despesa_atual)
                    ...
                    if self.salario.get_gasto_maximo() < self.salario.get_valor_despesa_total_gastos():
                        messagebox.showerror(
                            "Erro", "Ultrapassou do limite máximo de gastos")
                    elif x <= self.salario.get_valor_despesa_total_gastos():
                        messagebox.showwarning(
                            "Aviso", "Está próximo de ultrapassar o limite máximo de gastos")
                    else:
                        conn.commit()
                        conn.close()
                    ...
                    if self.orcamento_atual < 0:
                        messagebox.showerror(
                            "Erro", "Ultrapassou do limite do seu orçamento")
                    elif self.orcamento_atual <= self.orcamento_inicial*0.1:
                        messagebox.showwarning(
                            "Aviso", "Está próximo de ultrapassar o limite do seu orçamento")
                    else:
                        conn.commit()
                        conn.close()
                        self.salario.set_orcamento(self.orcamento_atual)
    ...
    def confirmar_adicoes(self):
        ...
        if c.execute("SELECT * FROM user_data WHERE Nome_do_utilizador = ?", (self.nome_actual,)).fetchone()[3] == None:
            c.execute("UPDATE user_data SET Categoria_de_despesa = ? WHERE Nome_do_utilizador = ?",
                      (self.temp_categoria_de_despesa, self.nome_actual,))
            c.execute("UPDATE user_data SET Descricao_de_despesa = ? WHERE Nome_do_utilizador = ?",
                      (self.temp_descricao_de_despesa, self.nome_actual,))
            c.execute("UPDATE user_data SET Valor_da_despesa = ? WHERE Nome_do_utilizador = ?",
                      (self.temp_valor_da_despesa, self.nome_actual,))
            c.execute("UPDATE user_data SET Data_da_despesa = ? WHERE Nome_do_utilizador = ?",
                      (self.temp_data_da_despesa, self.nome_actual,))
        else:
    ...
    def consultar_despesas(self):
        ...
        self.saved_table = c.execute(
            "SELECT * FROM user_data WHERE Nome_do_utilizador = ?", (self.nome_actual,)).fetchall()
        ...
        [self.categorias_de_despesa.append(
            x) for x in temp_categoria if x not in self.categorias_de_despesa]
        self.data_da_despesa = []
        [self.data_da_despesa.append(
            x) for x in temp_data if x not in self.data_da_despesa]
        ...
        for i in range(len(self.records)):
            ...
            for k in self.records[i]:
                consultar_label = tk.Label(self.nova_janela, text=k, bg="white", foreground="black", font=(
        ...
        self.drop_categoria_de_despesa = tk.OptionMenu(
            self.nova_janela, self.clicked_categoria_de_despesa, *self.categorias_de_despesa).grid(row=1, column=0)
        self.drop_valor_da_despesa = tk.OptionMenu(
            self.nova_janela, self.clicked_valor_da_despesa, *ascendente_descendente).grid(row=1, column=2)
        self.drop_data_da_despesa = tk.OptionMenu(
            self.nova_janela, self.clicked_data_da_despesa, *self.data_da_despesa).grid(row=1, column=3)

        self.button_configure = tk.Button(self.nova_janela, text="Configurar", bg="#2e5448", foreground="white", font=(
            "Arial", 12), width=10, command=self.confirmar_configuracao, padx=5)
    ...
    def confirmar_configuracao(self):
        ...
        if self.clicked_categoria_de_despesa.get() != "----" and self.clicked_data_da_despesa.get() == "----":
            for i in range(len(self.records)):
                tabela_adicoes.remove(tabela_adicoes.find(
                    self.clicked_categoria_de_despesa.get(), None, 0, None))

        if self.clicked_categoria_de_despesa.get() == "----" and self.clicked_data_da_despesa.get() != "----":
            for i in range(len(self.records)):
                tabela_adicoes.remove(tabela_adicoes.find(
                    None, self.clicked_data_da_despesa.get(), None, 3))

        if self.clicked_categoria_de_despesa.get() != "----" and self.clicked_data_da_despesa.get() != "----":
            for i in range(len(self.records)):
                tabela_adicoes.remove(tabela_adicoes.find(
                    self.clicked_categoria_de_despesa.get(), self.clicked_data_da_despesa.get(), 0, 3))

        if self.clicked_valor_da_despesa.get() != "----":
            tabela_adicoes.mergeSort(
                tabela_adicoes.head, self.clicked_valor_da_despesa.get())
        ...
        if self.clicked_categoria_de_despesa.get() != "----" or self.clicked_data_da_despesa.get() != "----" or self.clicked_valor_da_despesa.get() != "----":
            ...
            for i in range(len(self.records)):         # Limpa a tabela
                j = 0
                for k in self.records[i]:
                    consultar_label = tk.Label(self.nova_janela, text='', bg="white", foreground="black", font=(
                        "Arial", 12), width=25, height=1)
                    consultar_label.grid(row=i+3, column=j, columnspan=1)
                    j += 1
            ...
            for i in range(tabela_adicoes.size):
                ...
                for j in range(len(tabela_adicoes.head.element)):
                    consultar_label = tk.Label(self.nova_janela, text=tabela_adicoes.head.element[j], bg="white", foreground="black", font=(
                        "Arial", 12), width=25, height=1)
                    consultar_label.grid(row=i+3, column=j, columnspan=1)
    ...
    def orcamento_mensal(self):
        ...
        tk.Label(self.nova_janela, text="Indique-nos o seu orçamento mensal disponível:",
        ...
        self.orcamento_mensal_button = tk.Button(self.nova_janela, text="Adicionar", bg="#2e5448", foreground="white", font=(
            "Times New Roman", 14), width=10, command=self.adicionar_salario)
    ...
    def limitar_gastos(self):
        ...
        tk.Label(self.nova_janela, text="Defina um gasto máximo", bg="#92e3a9", font=(
        ...
        self.gastos_maximos_button = tk.Button(self.nova_janela, bg="#2e5448", foreground="white", text="Confirmar", font=(
            "Arial", 12), width=10, command=self.gastar)
        ...
        if self.salario != float:
            try:
                ...
                if self.initial_salario != 0:
                    ...
                    c.execute(
                        "DELETE FROM user_data WHERE Nome_do_utilizador = ? AND Orcamento_actual = ''", (self.nome_actual,))
                conn.commit()
                conn.close()
                if self.initial_salario != 0:
                ...
                messagebox.showerror(
                    "Erro", "Digite o seu salário em valor númerico")
        ...
        if self.gastos != int or float:
            try:
                ...
                if self.gastos > self.salario.get_orcamento():
                    ...
                    self.salario.set_gasto_maximo(self.gastos)
                    messagebox.showinfo(
                        "", "Gasto máximo definido com sucesso.")
                    self.nova_janela.destroy()
            except ValueError:
                messagebox.showerror(
                    "ERRO", "O orçamento tem que ser dado númericamente!")
    ...
    def analise(self):
        ...
        tk.Label(self.nova_janela, text="Orçamento inicial", bg="#92e3a9", font=(
            "Times New Roman", 15)).grid(row=0, column=0, sticky="w")
        tk.Label(self.nova_janela, text=str(valor_inicial), bg="white", foreground="black", font=(
            "Times New Roman", 12), width=25, height=1).grid(row=0, column=1)
        tk.Label(self.nova_janela, text="Orçamento actual", bg="#92e3a9", font=(
            "Times New Roman", 15)).grid(row=1, column=0, sticky="w")
        tk.Label(self.nova_janela, text=str(valor_actual), bg="white", foreground="black", font=(
            "Times New Roman", 12), width=25, height=1).grid(row=1, column=1)
        tk.Label(self.nova_janela, text="Categoria que mais gasta", bg="#92e3a9", font=(
            "Times New Roman", 15)).grid(row=2, column=0, sticky="w")
        tk.Label(self.nova_janela, text=categoria_gasto_max, bg="white", foreground="black", font=(
            "Times New Roman", 12), width=25, height=1).grid(row=2, column=1)
        tk.Label(self.nova_janela, text=self.tabela.sugestao(categoria_gasto_max), bg="#92e3a9",
                 foreground="black", font=("Times New Roman", 12), width=25, height=1).grid(row=3)

        for i in range(len(self.categorias_de_despesa)-1):
            tk.Label(self.nova_janela, text=self.categorias_de_despesa[i], bg="#92e3a9", foreground="black", font=(
                "Arial", 12), width=25, height=1).grid(row=i+4, column=0)
            tk.Label(self.nova_janela, text=str(self.tabela.media_da_categoria(
                self.categorias_de_despesa[i])), bg="#92e3a9", foreground="black", font=("Arial", 12), width=25, height=1).grid(row=i+4, column=1)
